<template>
  <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
    <button
      ...
      class="group p-4 rounded-lg border border-gray-200 bg-white hover:border-blue-500 hover:shadow-lg hover:scale-105 transition-all duration-200"
    >
      <!-- Icon Preview -->
      <svg
      ...
      <div class="text-center">
        <p class="text-xs font-medium text-gray-900 line-clamp-1">{{ icon.label }}</p>
        <p class="text-xs text-gray-600 line-clamp-1">{{ icon.name }}</p>
      </div>
    </button>

    <!-- Empty State -->
    <div v-if="icons.length === 0" class="col-span-full text-center py-12">
      <p class="text-gray-500 text-sm">Keine Icons gefunden</p>
    </div>
  </div>
</template>
